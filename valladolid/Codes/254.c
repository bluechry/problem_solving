/* @JUDGE_ID: andrea 254 C "Simple Iteration" */
...
int	 integer_ConvertToSmall(Integer* );
...
void Integer_Div(Integer*, Integer*, Integer*, Integer* );

int  Integer_Compare(Integer* , Integer* );
void Integer_Conv(int, Integer*, Integer* );
...
int	integer_ConvertToSmall(Integer* in) 
{
	int i;
	int result;
...
int Integer_IsZero(Integer* in)
{
	integer_JustifyZero(in);
...
void Integer_Sub(Integer* op1, Integer* op2, Integer* result)
{
	...
	/*
 	 *	(+A) - (+B)
 	 */
	if (op1->sign == PLUS && op2->sign == PLUS) {
		
		comp = Integer_Compare(op1, op2);
		...
		for (i = 0; i < op1->num_digits; i++) {
			if (op1->digits[i] >= op2->digits[i]) {
				result->digits[i] = op1->digits[i] - op2->digits[i];
			}
			else {
				result->digits[i] = op1->digits[i] + 10 - op2->digits[i];
				op1->digits[i + 1] -= 1;
			}
		}
	...
	/*
 	 *	(+A) - (-B)
 	 */
	else if (op1->sign == PLUS && op2->sign == MINUS) {
		op2->sign = PLUS;
		Integer_Add(op1, op2, result);
		op2->sign = MINUS;
	}
	/*
 	 *	(-A) - (+B)
 	 */
	else if (op1->sign == MINUS && op2->sign == PLUS) {
		op2->sign = MINUS;
		Integer_Add(op1, op2, result);
		op2->sign = PLUS;
	}
	/*
 	 *	(-A) - (-B)
 	 */
	else if (op1->sign == MINUS && op2->sign == MINUS) {
		op2->sign = PLUS;
		Integer_Sub(op2, op1, result);
		op2->sign = MINUS;
	}
...
void Integer_Div(Integer* op1, Integer* op2, Integer* result, Integer* remainder)
{
	int 	i, op1_sign, op2_sign;
	Integer temp, row;
	...
	if (Integer_Compare(op1, op2) == 1 ) {
		*remainder = *op1;
		...
		for (i = op1->num_digits - 1; i >= 0; --i ) {
			...
			row.digits[0] = op1->digits[i];

			while (Integer_Compare(&row, op2) <= 0) {
				result->digits[0]++;
				Integer_Sub(&row, op2, &temp);
				row = temp;
			}

			if (i > 0) { 
				integer_ShiftDigits(&row, 1);
				integer_ShiftDigits(result, 1);
			}
		}
	...
	/*
	 * verify signs of the result and the remainder
	 */
	result->sign = op1->sign * op2->sign;
	...
	integer_JustifyZero(result);
	integer_JustifyZero(remainder);

	return;
}


void Integer_Conv(int base, Integer* in, Integer* result)
{
	Integer b, temp;
	Integer quot, remainder;
	int		in_sign, count;
	...
	count = 0;
	while (!Integer_IsZero(&temp)) {

		Integer_Div(&temp, &b, &quot, &remainder);	

		result->digits[count++] = integer_ConvertToSmall(&remainder); 
	
		temp = quot;	
	}

	result->num_digits = count;
	result->sign = in_sign;
...
int main()
{
	...
	while (scanf("%d %s\n", &n, &m[0]) != EOF) {
		...
		Integer_InitFromString(m, &temp);
		Integer_Conv(2, &temp, &bin);
